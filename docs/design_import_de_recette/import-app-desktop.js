/* Fresh — Import de recette (prototype interactif) */
(function () {
  "use strict";

  const MARMITON_URL = "https://www.marmiton.org/recettes/recette_poulet-au-curry-et-lait-de-coco_18569.aspx";

  // ── Catalogue (extrait du catalogue interne ~2600 articles) ──
  const CATALOG = [
    { name: "Blanc de poulet", icon: "beef" },
    { name: "Poulet entier", icon: "beef" },
    { name: "Cuisse de poulet", icon: "beef" },
    { name: "Oignon", icon: "salad" },
    { name: "Oignon rouge", icon: "salad" },
    { name: "Échalote", icon: "salad" },
    { name: "Ail", icon: "salad" },
    { name: "Gingembre", icon: "flame" },
    { name: "Curry", icon: "flame" },
    { name: "Curcuma", icon: "flame" },
    { name: "Paprika", icon: "flame" },
    { name: "Riz", icon: "wheat" },
    { name: "Riz basmati", icon: "wheat" },
    { name: "Riz thaï", icon: "wheat" },
    { name: "Pâtes", icon: "wheat" },
    { name: "Huile d'olive", icon: "droplet" },
    { name: "Huile de tournesol", icon: "droplet" },
    { name: "Sel", icon: "soup" },
    { name: "Poivre", icon: "soup" },
    { name: "Crème de coco", icon: "milk" },
    { name: "Lait", icon: "milk" },
    { name: "Crème fraîche", icon: "milk" },
    { name: "Tomate", icon: "salad" },
    { name: "Carotte", icon: "carrot" },
    { name: "Coriandre", icon: "salad" }
  ];
  ...
  function qtyShort(it) {
    if (it.eye) return it.eyeLabel || "à l'œil";
    const u = UNIT_SHORT[it.unit] || it.unit || "";
    return [it.qty, u].filter(Boolean).join(" ");
  }
  ...
  function statusBarClone() { return ""; }

  function counts() {
    const c = { ok: 0, warn: 0, danger: 0 };
    state.ings.forEach((i) => c[i.conf]++);
    return c;
  }
  ...
  function render(keepScroll) {
    const prev = document.getElementById("revScroll");
    const top = keepScroll && prev ? prev.scrollTop : 0;
    if (state.screen === "url") flow.innerHTML = viewUrl();
    else if (state.screen === "loading") flow.innerHTML = viewLoading();
    else if (state.screen === "error") flow.innerHTML = viewError();
    else if (state.screen === "review") flow.innerHTML = viewReview();
    else if (state.screen === "saved") flow.innerHTML = viewSaved();
    icons();
    postRender();
    if (keepScroll) { const ns = document.getElementById("revScroll"); if (ns) ns.scrollTop = top; }
  }
  ...
  function viewUrl() {
    return `
      <div class="scroll fade-in">
        <div class="nav">
          <div class="back" data-action="noop"><i data-lucide="arrow-left" class="lic"></i></div>
          <div class="title-wrap"><div class="title">Importer une recette</div><div class="sub">Depuis une page web</div></div>
        </div>
        <div class="body">
          <div class="card">
            <div class="intro-head">
              ...
              <div>
                <div class="t">Colle le lien d'une recette</div>
                <div class="d">On lit la page et on prépare la recette pour toi — tu n'as plus qu'à vérifier.</div>
              </div>
            </div>
            <div class="label" style="margin-top:16px;">Lien de la recette</div>
            <div class="field" id="urlField">
              <i data-lucide="globe" class="lic"></i>
              <input id="urlInput" type="url" inputmode="url" placeholder="https://www.marmiton.org/…" value="${esc(state.url)}" autocomplete="off" />
            ...
            <button class="btn green" style="margin-top:12px;" data-action="import"><i data-lucide="download" class="lic"></i> Importer la recette</button>
          ...
          <div class="try-note"><b>Essaie :</b> « Coller » remplit une recette Marmiton.<br/>Un lien d'un autre site déclenche l'écran d'erreur.</div>
        </div>
      </div>`;
  }
  ...
  function viewLoading() {
    return `
      <div class="scroll center fade-in">
        ...
        <div class="steps-list" id="loadSteps">
          <div class="sl-row wait" data-k="0"><span class="dot"><i data-lucide="circle" class="lic"></i></span> Page récupérée</div>
          <div class="sl-row wait" data-k="1"><span class="dot"><i data-lucide="circle" class="lic"></i></span> Titre, photo &amp; temps lus</div>
          <div class="sl-row wait" data-k="2"><span class="dot"><i data-lucide="circle" class="lic"></i></span> Rattachement des ingrédients</div>
          <div class="sl-row wait" data-k="3"><span class="dot"><i data-lucide="circle" class="lic"></i></span> Étapes de préparation</div>
        </div>
      </div>`;
  }
  ...
  function viewError() {
    return `
      <div class="scroll fade-in" style="display:flex;flex-direction:column;">
        <div class="nav">
          <div class="back" data-action="to-url"><i data-lucide="arrow-left" class="lic"></i></div>
          <div class="title-wrap"><div class="title">Importer une recette</div></div>
        </div>
        <div class="center" style="flex:1;">
          ...
          <div style="width:100%;max-width:300px;margin-top:24px;display:flex;flex-direction:column;gap:10px;">
            <button class="btn green" data-action="to-url"><i data-lucide="rotate-ccw" class="lic"></i> Essayer une autre URL</button>
            <button class="btn ghost" data-action="manual"><i data-lucide="square-pen" class="lic"></i> Saisir la recette à la main</button>
          </div>
        </div>
      </div>`;
  }
  ...
  function viewReview() {
    const c = counts();
    return `
      <div class="scroll fade-in" id="revScroll">
        <div class="nav">
          <div class="back" data-action="to-url"><i data-lucide="arrow-left" class="lic"></i></div>
          <div class="title-wrap"><div class="title">Vérifier la recette</div><div class="sub">Corrige si besoin, puis enregistre.</div></div>
        </div>
        <div class="body">
          ...
          <div class="rev-photo">
            ...
            <button class="change" data-action="toast" data-msg="Photo — à brancher"><i data-lucide="camera" class="lic"></i> Changer</button>
          </div>
          ...
          <div class="meta-grid">
            ...
            <div class="bubble yellow"><div class="ic"><i data-lucide="gauge" class="lic"></i></div><div class="v" style="font-size:18px;">${esc(state.diff)}</div><div class="l">Difficulté</div></div>
          </div>
          ...
          <div class="tags">
            ...
            <span class="tag add" data-action="add-tag"><i data-lucide="plus" class="lic"></i> Tag</span>
          </div>
          ...
          <div class="sec">
            <div class="sec-h">
              ...
              <h3>Ingrédients</h3>
              <span class="count">${state.ings.length}</span>
            </div>
          ...
          <div class="sec">
            <div class="sec-h">
              ...
              <h3>Préparation</h3>
              <span class="count">${state.steps.length} étape${state.steps.length > 1 ? "s" : ""}</span>
            </div>
        ...
        <div class="action-bar">
          ...
          <button class="btn green" data-action="save"><i data-lucide="check" class="lic"></i> Enregistrer</button>
        </div>
      </div>`;
  }

  function recapHtml(c) {
    const done = c.danger === 0 && c.warn === 0;
    if (done) {
      return `<div class="recap"><div class="row">
        <span class="item ok"><i data-lucide="check-circle-2" class="lic"></i> ${c.ok} rattachés</span>
      </div><div class="note allgood"><i data-lucide="sparkles" class="lic"></i> Tout est rattaché — prêt à enregistrer.</div></div>`;
    }
    return `<div class="recap"><div class="row">
  ...
  function stepHtml(st, i) {
    ...
    return `<div class="step" data-id="${st.id}">
      ...
      <div class="sc">
        ...
        <div class="step-ings">${chips}<button class="ing-pick-btn ${menuOpen ? "active" : ""}" data-action="step-pick" data-id="${st.id}"><i data-lucide="carrot" class="lic"></i> ${linked.length ? "Modifier" : "Lier des ingrédients"}</button></div>
    ...
    return `
      <div class="scroll center fade-in">
        <div class="saved-badge">
          <div class="ring"><img src="mascot/chef-happy-t.png" alt="Chef Fresh" /></div>
          <div class="tick"><i data-lucide="check" class="lic"></i></div>
        </div>
        ...
        <div class="saved-sum" style="width:100%;max-width:300px;">
        ...
        ${c.danger || c.warn ? `<div class="err-url" style="background:var(--yellow-soft);color:var(--amber);margin-top:16px;"><i data-lucide="info" class="lic"></i> <span>${c.danger + c.warn} ingrédient(s) restaient à vérifier</span></div>` : ""}
        <div style="width:100%;max-width:300px;margin-top:24px;display:flex;flex-direction:column;gap:10px;">
  ...
  function renderSheet(focusCreate) {
    const it = state.ings.find((x) => x.id === state.sheetId);
    const ed = state.editing;
    if (!it || !ed) return;
    const q = ed.search.trim().toLowerCase();
    let results = CATALOG;
    if (q) results = CATALOG.filter((c) => c.name.toLowerCase().includes(q));
    ...
    const createBlock = (it.conf === "danger" || (q && noExact))
      ? `<div class="create-cta">
          <div class="ic"><i data-lucide="plus-circle" class="lic"></i></div>
          <div class="tx"><div class="t">Créer « ${esc(q || missName)} »</div><div class="d">Ajouter au catalogue et rattacher</div></div>
          <button class="go" data-action="create"><i data-lucide="plus" class="lic"></i> Créer</button>
        </div>` : "";

    const list = results.length
      ? results.map((c) => `<div class="cat-opt ${ed.art === c.name ? "sel" : ""}" data-action="pick" data-name="${esc(c.name)}">
          <div class="ci"><i data-lucide="${c.icon}" class="lic"></i></div>
          <div class="nm">${esc(c.name)}</div>
          <span class="ck"><i data-lucide="check" class="lic"></i></span>
        </div>`).join("")
    ...
    sheet.innerHTML = `
      ...
      <div class="sh-top">
        <div>
          <div class="sh-title">Corriger l'ingrédient</div>
          <div class="sh-brut">Détecté : « ${esc(it.brut)} »</div>
        </div>
      ...
      <div class="sheet-scroll">
        ...
        <div class="field" style="background:var(--sub);"><i data-lucide="search" class="lic"></i>
          <input id="catSearch" placeholder="Rechercher un article…" value="${esc(ed.search)}" autocomplete="off" />
        </div>
        ${createBlock}
        <div class="cat-list" style="margin-top:10px;">${list}</div>

        <div class="sh-label">Quantité</div>
        <div class="eye-toggle ${ed.eye ? "on" : ""}" data-action="toggle-eye">
          <span class="ic"><i data-lucide="eye" class="lic"></i></span>
          <div class="tx"><div class="t">Quantité « à l'œil »</div><div class="d">Sans chiffre — filet, pincée, au goût</div></div>
          <span class="sw"></span>
        </div>
        ${ed.eye
          ...
          : `<div class="qty-row" style="margin-top:10px;">
              <div class="qty-in"><input id="qtyIn" inputmode="decimal" placeholder="0" value="${esc(ed.qty)}" /></div>
              <select class="unit-sel" id="unitIn">${UNITS.map((u) => `<option ${u === ed.unit ? "selected" : ""}>${u}</option>`).join("")}</select>
            </div>`}
      </div>
      <div class="sheet-foot">
        <button class="btn ghost" data-action="close-sheet">Annuler</button>
        <button class="btn green" data-action="apply"><i data-lucide="check" class="lic"></i> Valider</button>
      </div>`;
    icons();

    const cs = document.getElementById("catSearch");
    if (cs) {
      cs.addEventListener("input", (e) => { state.editing.search = e.target.value; renderSheet(); });
      if (focusCreate) { cs.focus(); }
    }
    const qi = document.getElementById("qtyIn");
    if (qi) qi.addEventListener("input", (e) => { state.editing.qty = e.target.value; });
    const ui = document.getElementById("unitIn");
    if (ui) ui.addEventListener("change", (e) => { state.editing.unit = e.target.value; });
  }
  ...
  function applySheet() {
    ...
    render();
    const c = counts();
    if (c.danger + c.warn === 0) toast("Tout est rattaché");
    else toast("Ingrédient mis à jour");
    flashRecap();
  }

  function flashRecap() {
    const r = document.getElementById("recap");
    if (r) { r.style.transition = "none"; r.style.background = ""; }
  }
  ...
  function runLoading(gotoError) {
    ...
    let k = 0;
    function setRow(i, cls, ic) {
      const r = rows[i]; if (!r) return;
      r.className = "sl-row " + cls;
      r.querySelector(".dot").innerHTML = `<i data-lucide="${ic}" class="lic"></i>`;
      icons();
    }
    setRow(0, "active", "loader-2"); rows[0] && rows[0].querySelector(".lic") && rows[0].querySelector(".lic").classList.add("spin");
    ...
    const iv = setInterval(() => {
      ...
      k++;
      if (k < rows.length) {
        setRow(k, "active", "loader-2");
        const sp = rows[k].querySelector(".lic"); sp && sp.classList.add("spin");
      } else {
        clearInterval(iv);
        setTimeout(() => {
          if (gotoError) { state.screen = "error"; }
          else { seed(); state.screen = "review"; }
          render();
        }, 480);
      }
    }, 620);
  ...
  // ── Actions (délégation) ──────────────────
  function hostOf(u) { try { return new URL(u).host.replace(/^www\./, "") + new URL(u).pathname.replace(/\/$/, "").slice(0, 22); } catch (e) { return u ? u.replace(/^https?:\/\//, "").slice(0, 30) : ""; } }
  ...
  document.addEventListener("click", (e) => {
    ...
    if (a === "paste") {
      ...
      state.url = MARMITON_URL; if (inp) inp.value = MARMITON_URL;
      document.getElementById("urlField").classList.remove("err");
      document.getElementById("urlErr").classList.remove("show");
    } else if (a === "import") {
      const inp = document.getElementById("urlInput");
      state.url = (inp ? inp.value : state.url).trim();
      if (!state.url) {
        document.getElementById("urlField").classList.add("err");
        document.getElementById("urlErr").classList.add("show");
        return;
      }
      ...
      const it = state.ings.find((x) => x.id === +el.dataset.id);
      if (it) { it.conf = "ok"; delete it.hint; render(); toast("Rattachement confirmé"); }
    } else if (a === "close-sheet") {
      closeSheet();
    } else if (a === "pick") {
      state.editing.art = el.dataset.name; renderSheet();
    } else if (a === "create") {
      const it = state.ings.find((x) => x.id === state.sheetId);
      ...
      state.editing.art = pretty; renderSheet(); toast("Article créé au catalogue");
    } else if (a === "toggle-eye") {
      state.editing.eye = !state.editing.eye; renderSheet();
    } else if (a === "eye-label") {
      state.editing.eyeLabel = el.dataset.l; renderSheet();
    } else if (a === "apply") {
      applySheet();
    } else if (a === "add-step") {
      const id = Math.max(0, ...state.steps.map((s) => s.id)) + 1;
      state.steps.push({ id, t: "", d: "– min", ings: [] }); render();
      const nodes = document.querySelectorAll(".stxt"); const last = nodes[nodes.length - 1];
      if (last) { last.focus(); }
    } else if (a === "del-step") {
      state.steps = state.steps.filter((s) => s.id !== +el.dataset.id); render();
    } else if (a === "step-pick") {
      const id = +el.dataset.id;
      state.openStepMenu = state.openStepMenu === id ? null : id;
      render(true);
    } else if (a === "step-tog") {
      ...
      if (st) {
        ...
        st.ings = st.ings.includes(ing) ? st.ings.filter((x) => x !== ing) : [...st.ings, ing];
        state.openStepMenu = st.id;
        render(true);
      }
      ...
      state.tags.splice(+el.dataset.i, 1); render();
    } else if (a === "add-tag") {
      ...
      if (next) { state.tags.push(next); render(); } else toast("Plus de suggestions");
    } else if (a === "save") {
      state.openStepMenu = null; state.screen = "saved"; closeSheet(); render();
    } else if (a === "view-recipe") {
      toast("Ouverture de la recette…");
    } else if (a === "toast") {
      toast(el.dataset.msg || "");
    }
  });

  // contenteditable bindings + URL enter
  function postRender() {
    flow.querySelectorAll("[data-bind]").forEach((el) => {
      el.addEventListener("blur", () => { state[el.dataset.bind] = el.textContent.trim(); });
    });
    flow.querySelectorAll(".stxt[data-step]").forEach((el) => {
  ...
  function wireStepDnD() {
    const container = flow.querySelector(".steps");
    if (!container) return;
    ...
    container.querySelectorAll(".step").forEach((stepEl) => {
      ...
      stepEl.addEventListener("dragstart", (e) => {
        ...
        try { e.dataTransfer.setData("text/plain", stepEl.dataset.id); } catch (x) {}
      });
    ...
    container.addEventListener("dragover", (e) => {
      ...
      if (ref == null) container.appendChild(dragging);
      else container.insertBefore(dragging, ref);
    });

    function commitStepOrder() {
      const ids = [...container.querySelectorAll(".step")].map((el) => +el.dataset.id);
      state.steps.sort((a, b) => ids.indexOf(a.id) - ids.indexOf(b.id));
      render(true);
    }
  }
  ...
  function applyTheme(t) {
    ...
    localStorage.setItem("fresh-import-theme", t);
    ti.setAttribute("data-lucide", t === "dark" ? "sun" : "moon");
    tl.textContent = t === "dark" ? "Clair" : "Sombre";
    icons();
  }
